...
- 已完成：新建 `src/game/world/shop-world.js`，先承接 `drawShopWordOfMouthWorldNote`、`drawShopWordOfMouthMissingShelfWorldNote` 与 `drawShopWordOfMouthReadyShelfEchoWorldNote` 的“铺前市闻 / 来帖缺货 / 头排备齐”店铺口碑 world note 纯绘制实现；`src/game.js` 保留同名兼容包装、`settings.reducedMotion`、`drawCanvasCard` 与 `drawShopWeatherShelfGoodIcon` 注入，后续同域店铺世界卡可以继续往这个模块归并。
