...
- 已完成：新建 `src/game/world/shop-customer-journey-world.js`，承接 `drawShopFirstSaleActionTrailWorld`、`drawShopFirstSaleKeepsakeWorld`、`drawShopReturningTrailWorld`、`drawShopThoughtRouteWorldCard`、`drawShopCustomerLessonMorningFollowupWorld` 与 `drawShopCustomerLessonVerificationEchoWorld` 的店铺“首单动作链 / 顾客留痕 / 回头客轨迹 / 顾客判断续路线 / 经验回声”纯绘制实现；`src/game.js` 继续保留 `spec` 组装、focus 状态判断、`settings.reducedMotion`、`accent` 与 `drawCanvasCard` 注入，`tools/verify.mjs` 也已同步纳入新模块审计。
