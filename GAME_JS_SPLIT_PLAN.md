...
- 已完成：`drawLeizhuWindGuide` 的“雷竹风引/护送旧路可交互提示卡”Canvas 绘制实现继续拆到 `src/game/world/spirit-world.js`，`leizhuWindGuideSpec`、Canvas 命中、聚焦状态、palette/motion/glyph/card 注入仍留在 `src/game.js`。
...
- 已完成：`drawSpiritSproutTimelineWorldCard` 的“成精三步线索/时间线提示卡”Canvas 绘制实现继续拆到 `src/game/world/spirit-world.js`，`spiritSproutTimelineWorldSpec`、Canvas 命中、聚焦状态、motion/bob/active 与 `drawCanvasCard` 注入仍留在 `src/game.js`。
- 已完成：`drawSpiritSproutForeshadowTrailWorld` 的“第一夜成精伏笔灯/四步路线提示卡”Canvas 绘制实现继续拆到 `src/game/world/spirit-world.js`，`spiritSproutForeshadowTrailWorldSpec`、Canvas 命中、聚焦状态、motion/bob/pulse/focused/focusStepKey 与 `drawCanvasCard` 注入仍留在 `src/game.js`。
...
- 已完成：新建 `src/game/world/waterway-world.js`，承接 `drawQingboWaterFreshRestockWorldNote`、`drawQingboFirstSaleRestockSeedWorldNote`、`drawLingchiWaterFreshMenuWorldNote`、`drawWaterFreshRegularPledgeWorldNote`、`drawWaterFreshMenuRegularReasonWorldNote`、`drawQingheWaterFreshReturnOrderWorldNote`、`drawQingheWaterwayPreludeWorldNote`、`drawWaterFreshReturnToWaterwayReasonWorldNote`、`drawQingheLotusBasinTradeDispatchWorldNote`、`drawQingheLotusBasinFollowupOrderWorldNote`、`drawLotusBasinReturnFollowupReasonWorldNote`、`drawQingheLotusBasinLongOrderWorldNote`、`drawQingheLotusBasinStandingOrderWorldNote`、`drawQingheWaterwayAfterwordWorldNote` 与 `drawQingheWaterwayTownRumorWorldNote` 的清河水路 world note 纯绘制实现；`src/game.js` 仅保留同名兼容包装、focus 状态判断、`settings.reducedMotion` / `drawCanvasCard` 注入与既有调用点。
- 已完成：新建 `src/game/world/shop-world.js`，先承接 `drawShopWordOfMouthWorldNote`、`drawShopWordOfMouthMissingShelfWorldNote`、`drawShopWordOfMouthReadyShelfEchoWorldNote`，并继续下沉 `drawShopWordOfMouthSaleEchoWorld`、`drawShopWordOfMouthSaleReasonWorld`、`drawShopWordOfMouthFollowupRestockWorld`、`drawShopWordOfMouthMorningFollowupWorld`、`drawShopWordOfMouthRestockedMorningWorld` 与 `drawShopWordOfMouthRestockCaughtWorld` 的店铺口碑 world note / 成交回响 / 续货清晨链条纯绘制实现；`src/game.js` 保留同名兼容包装、focus 状态判断、`settings.reducedMotion`、`drawCanvasCard` 与 `drawShopWeatherShelfGoodIcon` 注入，后续同域店铺世界卡可以继续往这个模块归并。
- 已完成：新建 `src/game/world/shop-analysis-world.js`，承接 `drawShopDiagnosisWorldBoard`、`drawShopCustomerReasonCompassWorld`、`drawCustomerThoughtBubbles` 与 `drawShopThoughtBubbleChainWorld` 的店铺“旧铺诊断 / 顾客三因 / 门口想法泡泡 / 想法串联线”纯绘制实现；`src/game.js` 继续保留 `spec` 组装、focus 状态判断、`settings.reducedMotion`、`SHOP_THOUGHT_BUBBLE_POSITIONS` 与 `drawCanvasCard` 注入，`tools/verify.mjs` 也已同步纳入新模块审计。
- 已完成：新建 `src/game/world/shop-customer-journey-world.js`，承接 `drawShopTrialTheaterWorld`、`drawShopFirstCustomerThresholdWorld`、`drawShopFirstSaleLessonWorld`、`drawShopFirstSaleActionTrailWorld`、`drawShopFirstSaleKeepsakeWorld`、`drawShopReturningTrailWorld`、`drawShopThoughtRouteWorldCard`、`drawShopCustomerLessonMorningFollowupWorld` 与 `drawShopCustomerLessonVerificationEchoWorld` 的店铺“试营业小剧场 / 首客过门判断 / 首单经验回声 / 首单动作链 / 顾客留痕 / 回头客轨迹 / 顾客判断续路线”纯绘制实现；`src/game.js` 继续保留 `spec` 组装、focus 状态判断、`settings.reducedMotion`、`accent` 与 `drawCanvasCard` 注入，`tools/verify.mjs` 也已同步纳入新模块审计。
- 已完成：新建 `src/game/world/shop-morning-route-world.js`，承接 `shopWordOfMouthMorningFollowupWorldSpec` / `AtCanvasPoint`、`shopThoughtRouteMorningFollowupSpec`、`shopThoughtRouteMorningFollowupWorldSpec` / `AtCanvasPoint`、`shopThoughtRouteReadyMorningWorldSpec` / `AtCanvasPoint`、`shopThoughtRouteCaughtWorldSpec` / `AtCanvasPoint`、`shopThoughtRouteMissedWorldSpec` / `AtCanvasPoint`、`shopWordOfMouthRestockedMorningWorldSpec` / `AtCanvasPoint` 与 `shopWordOfMouthRestockCaughtWorldSpec` / `AtCanvasPoint` 的店铺“清晨续货 / 想法续线 / 备妥 / 接住 / 落空”world spec 与命中计算；`src/game.js` 继续保留 state 读取、`itemName` 注入、focus 状态、副作用日志和主世界分发，`tools/verify.mjs` 也已同步纳入新模块审计。
- 已完成：新建 `src/game/world/shop-targets-world.js`，承接 `shopCanvasTargets` 与 `shopAtCanvasPoint` 的店铺主世界点击目标装配 / 命中查找纯计算；`src/game.js` 继续保留 `normalizeShopOpeningState(state.shopOpeningState)`、各类 world spec helper 注入、`SHOP_THOUGHT_BUBBLE_POSITIONS`、focus 状态写入、副作用日志与 `focusShopFromCanvas` 分发，`tools/verify.mjs` 也已同步纳入新模块审计。
- 已完成：新建 `src/game/world/shop-prep-world.js`，承接 `drawShopShelfPrepWorldBoard`、`drawShopDailyGoodsEyeWorld`、`drawShopCustomerPickShadowWorld`、`drawShopSpiritGreeterWorld` 与 `drawShopRestockRunnerWorld` 的店铺“开铺准备 / 今日货眼 / 顾客影子 / 精怪迎客 / 补货跑腿”纯绘制实现；`src/game.js` 继续保留 `spec` 组装、focus 状态判断、`settings.reducedMotion`、`drawCanvasCard`、`drawShopWeatherShelfGoodIcon`、`drawShopCrowdPerson` 与 `pointOnPolyline` 注入，`tools/verify.mjs` 也已同步纳入新模块审计。
