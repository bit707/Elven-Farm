namespace XiannongCore.Combat {
  ...
  export interface DungeonFailureInsightRun extends CombatDungeonRun {
    floor?: number | string | null;
    bossReady?: boolean | number | string | null;
    bossPhase?: number | string | null;
    lastBossSkillId?: string | number | null;
    turn?: number | string | null;
    mechanicState?: DungeonMechanicState | null;
  }

  export interface DungeonFailureInsightSnapshot {
    failures?: number | string | null;
    revealedFloor?: number | string | null;
    pathBonus?: number | string | null;
    bossPhaseSeen?: number | string | null;
    bossGuard?: number | string | null;
    bossStrikeBonus?: number | string | null;
    bossFamiliarity?: number | string | null;
    bossBestPercent?: number | string | null;
    bossSkillIds?: Array<string | number | null | undefined> | null;
    overflowRelief?: number | string | null;
  }

  export interface DungeonFailureInsightPlanInput {
    key?: string | null;
    reason?: "overflow" | "battle" | "boss" | "retreat" | string | null;
    day?: number | string | null;
    previous?: DungeonFailureInsightSnapshot | null;
    run?: DungeonFailureInsightRun | null;
    dungeon?: {
      area_id?: string | null;
      floor_start?: number | string | null;
    } | null;
    mechanic?: {
      dungeon_id?: string | null;
    } | null;
    dungeonId?: string | null;
    floorStart?: number | string | null;
    mechanicId?: string | null;
    bossId?: string | null;
  }
  ...
  export interface DungeonFailureInsightPlan {
    ...
    bossGuard: number;
    bossStrikeBonus: number;
    bossFamiliarity: number;
    bossBestPercent: number;
    bossSkillIds: string[];
    bossNextSkillId: string;
  ...
  export function createDungeonRuntime(data: CombatRuntimeData): CombatRuntime {
    ...
    function dungeonFailureInsightPlan(input: DungeonFailureInsightPlanInput | null = null): DungeonFailureInsightPlan {
      ...
      const revealedFloor = Math.max(
        Number(previous.revealedFloor || 0),
        Number(run?.floor || input?.floorStart || input?.dungeon?.floor_start || 1),
      );
      const bossPhaseSeen = Math.max(
        Number(previous.bossPhaseSeen || 0),
        bossRelated ? Number(run?.bossPhase || 1) : 0,
      );
      ...
      const bossFamiliarity = bossRelated
        ? Math.min(5, Math.max(
          ...
          Math.ceil((100 - bossBestPercent) / 20) + Math.max(0, bossPhaseSeen - 1) + Math.min(1, bossSkillIds.length),
        ))
        : Number(previous.bossFamiliarity || 0);
      const basePathBonus = Math.min(3, 1 + Math.floor(failures / 2));
      const pathBonus = Math.max(
        Number(previous.pathBonus || 0),
        reason === "boss" ? Math.max(1, basePathBonus - 1) : basePathBonus,
      );
      const overflowRelief = Math.max(
        Number(previous.overflowRelief || 0),
        mechanicId === "dsm_004" && Number(run?.mechanicState?.overflow || 0) >= 4 ? 2 : 0,
      );
      const bossGuard = Math.max(
        Number(previous.bossGuard || 0),
        bossPhaseSeen > 0 ? Math.min(4, bossPhaseSeen + Math.floor(bossFamiliarity / 2)) : 0,
      );
      const bossStrikeBonus = Math.max(
        Number(previous.bossStrikeBonus || 0),
        bossFamiliarity > 0 ? Math.min(90, 18 * bossFamiliarity) : 0,
      );
      return {
        key: input?.key || "",
