...
(function (XiannongCore) {
    ...
    (function (Quests) {
        ...
        function createQuestRuntime(state, data, constants, hooks) {
            ...
            function configuredEventMainQuestActionPlan(event) {
                const plan = configuredEventExecutionPlan(event);
            ...
            function configuredEventHerbValleyFinishActionPlan(event) {
                const plan = configuredEventExecutionPlan(event);
                ...
                const actions = applies
                    ? [
                        ...
                        { kind: "update_missions" },
                        { kind: "check_quest_rewards" },
                        ...(shouldStartSpiritManor ? [{ kind: "start_spirit_manor_chapter_if_needed", eventNameKey: "event_name_main_0301", fallbackName: "百怪大院蓝图" }] : []),
            ...
            function configuredEventSpiritManorOverviewActionPlan(event) {
                const plan = configuredEventExecutionPlan(event);
                ...
                const actions = applies
                    ? [
                        ...
                        ...(shouldStartFactionOrder ? [{ kind: "start_faction_order_chapter_if_needed", eventNameKey: "event_name_main_0302", fallbackName: "商会来客" }] : []),
                    ]
                    : [];
                return {
                    applies,
            ...
            function configuredEventChapter4LuTruthActionPlan(event) {
                const plan = configuredEventExecutionPlan(event);
            ...
            function configuredEventChapter4FinalBanquetActionPlan(event) {
                const plan = configuredEventExecutionPlan(event);
                ...
                const actions = applies
                    ? [
                        ...
                        ...npcFavors.map((favor) => ({ kind: "add_npc_favor", npcId: favor.npcId, amount: favor.amount, source: favor.source })),
                        ...(fameAmount > 0 ? [{ kind: "add_fame", amount: fameAmount }] : []),
                        ...(firstFinish ? [{ kind: "grant_year2_starter_kit_if_needed" }] : []),
