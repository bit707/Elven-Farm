...
export function drawSpiritInteractionFeedbackWorld({
  ctx,
  width = 960,
  height = 640,
  feedback = null,
...
export function drawSpiritEvolutionFeedbackWorld({
  ctx,
  width = 960,
  height = 640,
  feedback = null,
  ...
  if (!ctx || !feedback || !spirit || !profile || activeCutscene || activeDialogueCount > 0) return false;
  const motion = reducedMotion ? 0 : now / 1000;
  const pulse = reducedMotion ? 0 : Math.sin(motion * 3.2) * 5;
  const progress = reducedMotion ? 1 : Math.min(1, Number(feedback.age || 0) / 1400);
  ...
  ctx.fillText(String(feedback.title || "").slice(0, 18), x + 206, y + 76);
  ctx.fillStyle = "#8f5f3f";
  ctx.font = "700 15px Microsoft YaHei";
  ctx.fillText(`${feedback.previousName} -> ${feedback.spiritName}`.slice(0, 22), x + 206, y + 102);
  ctx.fillStyle = "#5d6f65";
  ctx.font = "13px Microsoft YaHei";
  ctx.fillText(String(feedback.detail || "").slice(0, 30), x + 206, y + 128);
  ...
  ctx.fillStyle = "#b47d2f";
  ctx.font = "700 12px Microsoft YaHei";
  ctx.fillText(String(feedback.eventAction || "").slice(0, 22), x + 220, y + 188);
  ctx.fillStyle = "#17231d";
  ctx.font = "700 13px Microsoft YaHei";
  ctx.fillText(String(feedback.cta || "").slice(0, 30), x + 220, y + 208);
  ctx.fillStyle = "#8f5f3f";
  ctx.font = "11px Microsoft YaHei";
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    ctx.fillStyle = i % 2 ? "rgba(246, 240, 182, 0.82)" : `${accent}88`;
    ctx.beginPath();
...
export function drawSpiritFinaleFeedbackWorld({
  ctx,
  width = 960,
  height = 640,
  feedback = null,
  ...
  if (!ctx || !feedback || !spirit || !profile || activeCutscene || activeDialogueCount > 0) return false;
  const motion = reducedMotion ? 0 : now / 1000;
  const pulse = reducedMotion ? 0 : Math.sin(motion * 2.8) * 5;
  const progress = reducedMotion ? 1 : Math.min(1, Number(feedback.age || 0) / 1500);
  ...
  ctx.fillStyle = "#5d6f65";
  ctx.font = "13px Microsoft YaHei";
  ctx.fillText(String(feedback.sceneText || "").slice(0, 36), x + 198, y + 132);
  ctx.fillText(`“${String(feedback.dialogueText || "").slice(0, 30)}”`.slice(0, 36), x + 198, y + 154);
  ...
  ctx.fillStyle = accent;
  ctx.font = "700 12px Microsoft YaHei";
  ctx.fillText(String(feedback.effectText || "").slice(0, 24), x + 212, y + 190);
  ctx.fillStyle = "#17231d";
  ctx.font = "700 12px Microsoft YaHei";
