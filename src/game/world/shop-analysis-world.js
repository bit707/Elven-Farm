...
export function drawShopCustomerForecastWorldBoardWorld({
  ctx,
  spec = null,
  motion = 0,
  active = false,
  ...
  if (!ctx || !spec?.active || !spec.rect) return false;
  ...
  const pulse = reducedMotion ? 0 : Math.sin(motion * 2.15) * 1.8;
  const cardY = y + pulse;
  const good = spec.topGood || { itemName: labels.refillName || "补货", count: 0, missing: true };
  ...
  ctx.fillText(labels.windGlyph || "风", x + 22, cardY + 37);

  ctx.fillStyle = accent;
  ctx.font = "800 11px Microsoft YaHei";
  ctx.fillText(`${spec.title} · ${labels.clickable || "可点"}`, x + 60, cardY + 20);
  ...
  ctx.fillText(`${labels.featuredPrefix || "主推"} ${spec.itemText}`.slice(0, 21), x + 60, cardY + 55);
  ...
  ctx.fillStyle = accent;
  ctx.font = "800 9px Microsoft YaHei";
  ctx.fillText(`${labels.openingPrefix || "开铺："}${spec.openingLine}`.slice(0, 17), x + 18, cardY + 94);
  ctx.fillStyle = "#5d6f65";
  ctx.font = "9px Microsoft YaHei";
  ctx.fillText(`${labels.advicePrefix || "建议："}${spec.advice}`.slice(0, 20), x + 118, cardY + 94);
  ...
  drawShopWeatherShelfGoodIcon(ctx, good, x + width - 44, cardY + 19, 28, {
    ...
    pulse: reducedMotion ? 0 : Math.sin(motion * 2.3) * 1.2,
  });
  ctx.restore();
  return true;
}
...
export function shopCrowdHeatUiSpecWorld({
  liveFocus = null,
  heat = null,
} = {}) {
  const activeHeat = heat || shopCrowdHeatSpecWorld({ liveFocus });
  const buyers = Math.max(0, Number(liveFocus?.buyers || 0));
  const leavers = Math.max(0, Number(liveFocus?.leavers || 0));
  const themeScore = Math.max(0, Number(liveFocus?.themeScore || 0));
  const stateClass = activeHeat.heatTier === "hesitant"
    ? "warn"
    : buyers > 0
      ? "good"
      : "idle";
  const title = activeHeat.heatTier === "packed"
    ? "门口排起小队"
    : activeHeat.heatTier === "busy"
      ? "旧铺门口热起来了"
      : activeHeat.waterwayScene?.active
        ? "莲泽客船停在旧铺旁"
        : activeHeat.heatTier === "warm"
          ? "第一圈围观形成"
          : activeHeat.returningCount > 0
            ? "熟脸先把门口点热了"
            : leavers > 0
              ? "有人在门口犹豫离店"
              : "旧铺还在等第一波驻足";
  const detail = activeHeat.returningCount > 0
    ? `${activeHeat.topItemName || activeHeat.hotTagLabel || "这排货"}已经被昨天记住，熟脸回门正在替你把门口重新点热。`
    : activeHeat.waterwayScene?.active
      ? `${activeHeat.waterwayScene.sceneLine}。${activeHeat.waterwayScene.nextAction}`
      : activeHeat.topItemName
        ? `${activeHeat.topItemName} 已经挂上热卖牌，下一轮可以沿着这件货继续补同类陈列。`
        : leavers > 0
          ? `${activeHeat.topBlockerLabel || "顾客犹豫"}让门口热度散了一点，先修正这处短板更稳。`
          : activeHeat.hotTagLabel
            ? `货架正在围绕“${activeHeat.hotTagLabel}”等人驻足，先准备一件高匹配加工品。`
            : "先准备一件能讲清楚标签的商品，旧铺门口才会有第一圈人。";
  const chips = [
    { label: "排队", value: activeHeat.buyerCount, tone: buyers > 0 ? "good" : "idle" },
    { label: "围观", value: activeHeat.lookerCount, tone: activeHeat.lookerCount > 0 ? "good" : "idle" },
    { label: "熟脸", value: activeHeat.returningCount, tone: activeHeat.returningCount > 0 ? "good" : "idle" },
    { label: "水航客", value: activeHeat.waterwayScene?.visitorCount || 0, tone: activeHeat.waterwayScene?.active ? "good" : "idle" },
    { label: "犹豫离店", value: activeHeat.leaverCount, tone: leavers > 0 ? "warn" : "idle" },
