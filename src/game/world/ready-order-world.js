...
export function orderSeedPrepWorldBoardSpecWorld({
  width = 960,
  height = 640,
  rows = [],
  day = 1,
  ...
  return {
    key: `${day}:${rows.map((row) => `${row.orderId}:${row.seedId}:${row.plot.x},${row.plot.y}`).join("|")}`,
    day,
    rows,
    top,
    rect: { x, y, width: cardWidth, height: cardHeight },
    anchor,
    title: copy?.title || "主世界订单缺口可播种",
    headline: `${top.itemName} 还差 ${top.missingCount}`,
    detail: `${top.seedName} 可播 ${top.plotLabel}`,
    cta: copy?.cta || "缺口可下种 · 可点",
  };
}

export function orderSeedPrepWorldBoardAtCanvasPointWorld({ px, py, spec = null } = {}) {
  if (!spec?.rect) return null;
  const { rect } = spec;
  return (
    px >= rect.x
...
export function drawOrderSeedPrepWorldBoardWorld({
  ctx,
  spec = null,
  reducedMotion = false,
  motion = performance.now() / 1000,
  ...
  const { rect, rows, top, anchor } = spec;
  const safeMotion = reducedMotion ? 0 : motion;
  const bob = reducedMotion ? 0 : Math.sin(safeMotion * 1.78) * 2.2;
  const cardY = rect.y + bob;
  const active = focus?.day === day && focus?.key === spec.key;
  ...
  for (let leaf = 0; leaf < 3; leaf += 1) {
    const sway = reducedMotion ? 0 : Math.sin(safeMotion * 2 + leaf) * 2;
    ctx.beginPath();
