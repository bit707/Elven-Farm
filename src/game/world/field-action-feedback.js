...
export function drawHarvestFeedbackWorld({
  ctx,
  width = 960,
  height = 640,
  feedback = null,
  ...
  badgeForRoute = () => ({ fill: "#f6f0b6", stroke: "#b47d2f", text: "#8f5f3f" }),
  drawCanvasCard = null,
} = {}) {
  if (!ctx || !feedback || activeCutscene || activeDialogueCount > 0 || !drawCanvasCard) return false;
  const motion = reducedMotion ? 0 : now / 1000;
  const lift = reducedMotion ? 0 : Math.sin(motion * 3.4) * 4;
  const routeBadge = badgeForRoute(feedback.route);
  ...
  ctx.fillText(String(qualityText || "").slice(0, 28), x + 126, y + 106);
